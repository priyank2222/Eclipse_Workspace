...
public class Node {
    ...
    void rtinit(int nodename, int[] initial_lkcost) 
    {
        ...
        costs = new int[4][4];
        old_costs = new int[4];   //record the direct connect cost
        path = new int[4];   //record the path from this node to node 0,1,2,3
    ...
    void rtupdate(Packet rcvdpkt)
    {
        ...
        Packet rcvPacket = rcvdpkt;
        
        int tRoute[];
        int temp;
        
        int previousNode = rcvPacket.sourceid;
        int currentNode = rcvPacket.destid;
        ...
        for(int i = 0 ; i < 4 ; i++)
        {
            if(costs[previousNode][i] != rcvPacket.mincost[i])   // Value for any link has changed
            {
                change = true;
                costs[previousNode][i] = rcvPacket.mincost[i];
    ...
    void linkhandler(int linkid, int newcost)
    {
        ...
        System.out.println("Cost has beeen Changed : " + nodename + "-->" + linkid);
